...
def flatten_table(cursor, source_table_name):
    cursor.execute(
        """
        SELECT rows, snapshot_date
        FROM public.sales_salesrawtable
        WHERE "table" = %s
    ...
    rows, snapshot_date = cursor.fetchone()
    header, data = rows[0], rows[1:]

    columns = dedupe_columns([sanitize_column(name) for name in header])
    target_table = f"sales_{sanitize_column(source_table_name)}"
    ...
    primary_key_sql = ', PRIMARY KEY ("id")' if "id" in columns else ""

    cursor.execute(f'DROP TABLE IF EXISTS public."{target_table}"')
    cursor.execute(
        f'CREATE TABLE public."{target_table}" ({column_defs}, snapshot_date DATE{primary_key_sql})'
    )

    column_list = ", ".join(f'"{col}"' for col in columns) + ", snapshot_date"
    insert_rows = [
        tuple(convert_value(value, col_type) for value, col_type in zip(row, column_types)) + (snapshot_date,)
        for row in data
    ]
    execute_values(
        cursor,
    ...
    with SSHTunnelForwarder(
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute('SELECT DISTINCT "table" FROM public.sales_salesrawtable')
                source_tables = [row[0] for row in cursor.fetchall()]

                for source_table_name in source_tables:
                    target_table, row_count, snapshot_date = flatten_table(cursor, source_table_name)
                    print(f'{source_table_name} -> public."{target_table}": {row_count} rows (snapshot {snapshot_date})')
